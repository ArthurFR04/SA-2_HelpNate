  ...
  return (
    <Stack.Navigator>
      <Stack.Screen name="Feed2" component={FeedScreen} options={({ navigation }) => ({
        ...
        headerRight: () => (
          <View style={{ height: "100%", display: "flex", flexDirection: "row", alignItems: "center", marginRight: 10 }}>
            <Link to={{ screen: 'AllChats' }}>
              <Ionicons name="send" style={{ marginRight: 10, color: "#3c3c3c" }} size={34} />
            </Link>
            <Text style={{ opacity: 0 }}>...</Text>
            <Link style={{ marginTop: userInfo === undefined ?  0 : "auto"}} to={{ screen: 'Profile' }}>
              {userInfo === undefined ? <Ionicons name="person-circle-outline" style={{ marginRight: 10, color: "#3c3c3c"}} size={40} />
                : <Image source={{ uri: `https://helpnate.herokuapp.com/uploads/${userInfo.foto_perfil}` }} style={{ borderRadius: 25, width: 40, height: 40 }} resizeMode={"cover"} />
              }
            </Link>
          </View>
        ),
...
export const Stack_NewPost = () => (
  <Stack.Navigator>
    <Stack.Screen name="NewPost2" component={NewPostScreen} options={({ navigation }) => ({
      ...
      headerLeft: () => (
        <View style={{ display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "center", width: Dimensions.get('window').width, position: "absolute", left: 0 }}>
            <Link to={{ screen: 'Feed' }} style={{ position: 'absolute', left: "4%" }}>
              <FontAwesome name="home" style={{ marginLeft: 8, color: "#3c3c3c" }} size={40} />
            </Link>
            <View style={{ display: "flex", flexDirection: "row", alignItems: "center" }}>
              <Text style={{ color: "#3c3c3c", fontSize: 30, fontWeight: 'bold' }}>Help</Text>
              <Text style={{ color: "#fff", fontSize: 30, fontWeight: 'bold' }}>Nate</Text>
